...
public final class YearMonthSpan extends AbstractSpan<YearMonth, YearMonthSpan> implements TemporalSpan<YearMonth> {
    ...
    public static YearMonthSpan of(final Span<YearMonth, Duration> span) {
        Objects.requireNonNull(span, "span");
        if (span instanceof YearMonthSpan) {
            return (YearMonthSpan) span;
        }
        return of(span.getStart(), span.getDuration());
    }

    public static YearMonthSpan of(final YearMonth start, final Duration duration) {
        Objects.requireNonNull(start, "start");
        Objects.requireNonNull(duration, "duration");
        if (duration.isNegative()) {
            throw new IllegalArgumentException("duration is negative");
        }
        return new YearMonthSpan(start, duration);
    }

    private YearMonthSpan(final YearMonth start, final Duration duration) {
        super(start, duration);
    }

    @Override
    protected SpanFactory<YearMonth, Duration, YearMonthSpan> getFactory() {
        return YearMonthSpan::new;
    }

    @Override
    public YearMonthInterval toInterval() {
        return YearMonthInterval.of(getStart(), getDuration());
    }
